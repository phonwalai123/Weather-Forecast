...
class WeatherNetworkManager : WeatherServiceProtocol {
    
    func fetchCurrentLocationWeather(lat: String, lon: String, completion: @escaping (WeatherModel) -> ()) {
        let API_URL = "http://api.openweathermap.org/data/2.5/weather?lat=\(lat)&lon=\(lon)&appid=\(NetworkProperties.API_KEY)&units=metric"
    ...
    func fetchNextFiveWeatherForecast(city: String, completion: @escaping ([ForecastTemperature]) -> ()) {
        let formattedCity = city.replacingOccurrences(of: " ", with: "+")
        let API_URL = "http://api.openweathermap.org/data/2.5/forecast?q=\(formattedCity)&appid=\(NetworkProperties.API_KEY)&units=metric"
             ...
             URLSession.shared.dataTask(with: urlRequest) { [weak self] (data, response, error) in
                 ...
                 do {
                    ...
                    for day in 0...forecastWeather.list.count - 1 {
                        ...
                        let f = DateFormatter()
                        let weekday = f.weekdaySymbols[weekdaycomponent] // 0 Sunday 6 - Saturday
...
struct NetworkProperties {
    static let API_KEY = "cf50b8c41fa33fb66a4a5441e890bf36"
}
